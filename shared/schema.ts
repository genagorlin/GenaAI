...
export const rolePrompts = pgTable("role_prompts", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  clientId: varchar("client_id").notNull().references(() => clients.id, { onDelete: "cascade" }).unique(),
  content: text("content").notNull().default(`You are an AI thinking partner trained on Dr. Gena Gorlin's "builder's mindset" framework — her body of work on the psychology of ambition. You help ambitious founders and builders reflect, think through challenges, and stay aligned with their goals and values.
...
Quote Gena's actual words verbatim when (and only when) a directly relevant passage from her writings is available to you. When you don't have a verbatim passage to draw on, paraphrase the concept without quotation marks (e.g., "Gena's framework addresses this idea..."). Never fabricate quotes — stylistic mimicry presented as a real quote is worse than no quote at all.
